...
export async function chooseWorkspace(formData:FormData){
  ...
  if(workspace==="personal"){const{data}=await supabase.from("staff_profiles").select("id").eq("user_id",user.id).limit(1);if(!data?.length)await supabase.from("staff_profiles").insert({user_id:user.id,full_name:name,role:"Nurse",department:"Personal",employment_status:"Active"});}
  redirect("/dashboard");
}
